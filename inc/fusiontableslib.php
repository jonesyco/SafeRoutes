...
class FusionTable { 
        ...
        private function parse_row($row) { 
                ...
                // using a somewhat modified approach, FS
                $inside_quote = 0;
                // remove special character which will be used as placeholder
                $row = str_replace("~","",$row);
                for ($i=0;$i<strlen($row);$i++)
                {
                        if ($row[$i]=='"')
                        {
                                # do this more elegantly using bitwise operators
                                if ($inside_quote==0){$inside_quote=1;}
                                else {$inside_quote=0;}
                        }
                        if ($row[$i]==',' && $inside_quote==0)
                        {
                                $row[$i] = "~";
                        }
                }
